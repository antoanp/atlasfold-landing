...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "serviceEstablishmentPage" });
  const baseUrl = process.env.BASE_URL ?? "https://atlasfold.com";
  const path = "service-establishment";
...
export default async function ServiceEstablishmentPage({ params }: Props) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "serviceEstablishmentPage" });
  const baseUrl = process.env.BASE_URL ?? "https://atlasfold.com";
  ...
  const serviceSchema = {
    ...
    url: `${baseUrl}/${locale}/service-establishment`,
  };
