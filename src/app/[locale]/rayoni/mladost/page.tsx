...
const PATH = paths.areas.mladost;

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "mladostAreaPage" });
...
export default async function MladostAreaPage({ params }: Props) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "mladostAreaPage" });
  const baseUrl = process.env.BASE_URL ?? "https://atlasfold.com";
  ...
  const serviceSchema = {
    ...
    areaServed: {
      "@type": "Place",
      name: locale === "bg" ? "Младост, София" : "Mladost, Sofia",
      containedInPlace: {
        "@type": "City",
        ...
        <article className="py-16 lg:py-24">
          <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            ...
            <figure className="mt-10 overflow-hidden rounded-2xl border border-gray-100 bg-card shadow-sm">
              <Image
                src="/images/sofia_mladost_biznes_park.webp"
                alt={t("image1Alt")}
                width={1200}
                height={630}
            ...
            <LeadCtaSection namespace="mladostAreaPage" className="mt-14" />
