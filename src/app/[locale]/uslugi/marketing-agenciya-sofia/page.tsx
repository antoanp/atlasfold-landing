...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "marketingAgencyPage" });
  const baseUrl = process.env.BASE_URL ?? "https://atlasfold.com";
  const path = paths.services.marketingAgency;
...
export default async function MarketingAgencyPage({ params }: Props) {
  ...
  const serviceSchema = {
    ...
    url: `${baseUrl}/${locale}/${paths.services.marketingAgency}`,
  };
